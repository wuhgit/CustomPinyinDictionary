...
> 考虑到之前的操作模式过于麻烦，且目前绝大多数 Android 用户都通过 **Magisk** 进行 root，今后本词库的相关数据将通过 Magisk module 模式进行更新。借助于 Magisk 的模块更新机制，您可以第一时间在模块管理页面得到词库升级提示，使用体验会更好。

下载 `CustomPinyinDictionary_Gboard_Magisk.zip` 并通过 Magisk 应用进行模块安装。
